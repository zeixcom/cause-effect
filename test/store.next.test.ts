...
describe('store', () => {
	...
	describe('watch callbacks', () => {
		test('watched callback triggered only by direct store access', async () => {
			...
			await new Promise(resolve => setTimeout(resolve, 50))
			expect(rootStoreCounter).toBeGreaterThan(0)
		...
		test('store property addition/removal affects store watched callback', () => {
			...
			// Watch a specific user property
			const userEffect = createEffect(() => {
				store.users.get()
			})
			expect(usersStoreCounter).toBe(1) // Still 1 - nested access doesn't trigger root
