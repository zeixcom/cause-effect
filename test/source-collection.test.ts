...
describe('createSourceCollection', () => {
	test('should create a collection with initial values', () => {
		const col = createSourceCollection([1, 2, 3], () => () => {})
	...
	test('should create an empty collection', () => {
		const col = createSourceCollection<number>([], () => () => {})
	...
	test('should support at(), byKey(), keyAt(), indexOfKey()', () => {
		const col = createSourceCollection(
			[
				{ id: 'a', name: 'Alice' },
				{ id: 'b', name: 'Bob' },
			],
	...
	test('should support iteration', () => {
		const col = createSourceCollection([10, 20, 30], () => () => {})
	...
	test('should call start callback on first effect access', () => {
		...
		const col = createSourceCollection([1], () => {
			started = true
			return () => {
				cleaned = true
			}
	...
	test('should add items via applyChanges', () => {
		...
		const col = createSourceCollection<number>([], applyChanges => {
			apply = applyChanges
			return () => {}
		})
	...
	test('should skip applyChanges when changed is false', () => {
		let apply: ((changes: DiffResult) => void) | undefined
		const col = createSourceCollection([1], applyChanges => {
			apply = applyChanges
			return () => {}
		})
	...
	test('should support custom key config with string prefix', () => {
		const col = createSourceCollection([10, 20], () => () => {}, {
			keyConfig: 'item-',
		})
	...
	test('should support custom createItem factory', () => {
		...
		const col = createSourceCollection([5, 10], () => () => {}, {
	...
	test('should trigger effects on structural changes', () => {
		let apply: ((changes: DiffResult) => void) | undefined
		const col = createSourceCollection<string>([], applyChanges => {
			apply = applyChanges
			return () => {}
		})
