...
- **`MutableStore<T>` / `isMutableStore(x)`**: Non-deprecated 1.x bridge names for the mutable keyed-record type and its guard, ahead of the v2.0 taxonomy where `Store` is repurposed as the readonly base (today's `DerivedStore`). Same type and behavior as today's `Store`. `isMutableStore(x)` additionally requires the write capability, so — unlike the tag-based `isStore(x)`, which matches both — it rejects a `DerivedStore` returned by `deriveStore`.
...
- **`ListSource<T>` / `ListCallback<T>` / `ListChanges<T>` / `PerItemCallback<T, U>`**: Non-deprecated 1.5.0 bridge names for `deriveList`'s per-item source type, external-push watched callback, mutation-changes descriptor, and per-item transformation callback, ahead of the v2.0 taxonomy. Same type and behavior as today's `CollectionSource`, `CollectionCallback`, `CollectionChanges`, and `DeriveCollectionCallback`. All four are terminal 2.0 vocabulary — no further rename at the 2.0 boundary, unlike `DerivedList`/`DerivedStore`. `deriveList`'s own signature now uses the four new names internally.
- **`tools/codemod-v2.ts`**: New ts-morph codemod that rewrites consumer code to the v2.0 bridge names (`List`→`MutableList`, `isList`→`isMutableList`, `Collection`→`DerivedList`, `isCollection`→`isDerivedList`, `Store`→`MutableStore`, `isStore`→`isMutableStore`, `CollectionSource`→`ListSource`, `CollectionCallback`→`ListCallback`, `CollectionChanges`→`ListChanges`, `DeriveCollectionCallback`→`PerItemCallback`, `createCollection(cb, o?)`→`deriveList(seed, { ... })`). Meaning-preserving: output compiles and behaves identically before and after. Exact identifier match only; `--module` scopes which import declarations are rewritten (a substring match on the module specifier, defaulting to `cause-effect`). Renamed `isList` and `isStore` call sites are flagged for manual review — read-only `List` positions and call sites that rely on `isStore` matching a `DerivedStore` need a human decision the codemod cannot make.
...
- **`Store<T>` / `isStore(x)`**: Marked `@deprecated`. Use `MutableStore<T>` / `isMutableStore(x)` instead — same type and behavior today. `isStore` checks the shape tag only and matches a `DerivedStore` too; `isMutableStore` also requires the write capability. In v2.0, `Store` is repurposed as the readonly base (today's `DerivedStore`) and `isStore` narrows accordingly, so code that keeps using the old names will silently change meaning at that release.
- **`.deriveCollection(fn)`**: Marked `@deprecated` on `MutableList` and `DerivedList`. Use the top-level `deriveList(source, itemFn)` instead — `users.deriveCollection(f)` becomes `deriveList(users, f)`. Both the method form and the module-level `deriveCollection` (unexported by this release) are removed in v2.0.
...
- **`CollectionSource<T>` / `CollectionCallback<T>` / `CollectionChanges<T>` / `DeriveCollectionCallback<T, U>`**: Marked `@deprecated`. Use `ListSource<T>` / `ListCallback<T>` / `ListChanges<T>` / `PerItemCallback<T, U>` instead — same type and behavior today, and the terminal 2.0 name.
- **`DeriveCollectionOptions<T>`**: Marked `@deprecated`. Use `DeriveListOptions<T>` instead — every field it has, `DeriveListOptions` already has. Folds away entirely in v2.0; unlike the other four names in this group, it has no distinct v2.0 name of its own.
