...
- **`tools/codemod-v2.ts`**: New ts-morph codemod that rewrites consumer code to the v2.0 bridge names (`List`→`MutableList`, `isList`→`isMutableList`, `Collection`→`DerivedList`, `isCollection`→`isDerivedList`, `Store`→`MutableStore`, `isStore`→`isMutableStore`, `createCollection(cb, o?)`→`deriveList(seed, { ... })`). Meaning-preserving: output compiles and behaves identically before and after. Exact identifier match only; `--module` scopes which import declarations are rewritten (a substring match on the module specifier, defaulting to `cause-effect`). Renamed `isList` and `isStore` call sites are flagged for manual review — read-only `List` positions and call sites that rely on `isStore` matching a `DerivedStore` need a human decision the codemod cannot make.
...
- **`.deriveCollection(fn)`**: Marked `@deprecated` on `MutableList` and `DerivedList`. Use the top-level `deriveList(source, itemFn)` instead — `users.deriveCollection(f)` becomes `deriveList(users, f)`. Both the method form and the module-level `deriveCollection` (unexported by this release) are removed in v2.0; `CollectionSource<T>` renames to `ListSource<T>` at the same boundary (a manual rename — the codemod carries no rule for it).
