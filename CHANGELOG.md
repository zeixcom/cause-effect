...
- **`UnresolvableKeyError`**: New error class, thrown by the external-push form of `deriveList` when a `change`/`remove` entry cannot be resolved to a tracked key (see Fixed below). Forward-ported from 1.5.2.
...
- **`List<T>`/`isList(x)` and `Store<T>`/`isStore(x)` are repurposed as the readonly base — the 1.5.0 mutable meaning moves to `MutableList`/`isMutableList` and `MutableStore`/`isMutableStore`** (`src/nodes/list.ts`, `src/nodes/store.ts`): Completes the shape-indexed collapse for the two composite shapes, mirroring the `Signal`→`Cell` split above. `List`/`isList` now means what `DerivedList`/`isDerivedList` meant in 1.5.0 (the readonly type returned by `deriveList`); `Store`/`isStore` now means what `DerivedStore` meant (1.5.0 had no separate `isDerivedStore` guard — `isStore` already matched a derived store there, checking the shape tag only). The 1.x mutable meaning is `MutableList`/`isMutableList`/`MutableStore`/`isMutableStore`, unchanged from their 1.5.0 bridge names. `DerivedList`/`isDerivedList`/`DerivedStore` are removed — folded into the recycled `List`/`Store`/`isList` names. **This is a silent meaning flip, not a compile error**: code still on the 1.x `List<T>`/`Store<T>` meaning (mutable) type-checks against the new readonly meaning only where it doesn't call `.set()`/`.update()`, and `isStore(x)` — already permissive in 1.x — is unaffected, while `isList(x)` now returns `true` for a `deriveList` result it previously rejected. **Migration:** rename every mutable-intent `List`/`Store`/`isList` reference to `MutableList`/`MutableStore`/`isMutableList` before adopting 2.0; the codemod (`tools/codemod-v2.ts`) does this rename unconditionally, since it cannot tell a mutable position from a read-only one. See `MIGRATION-2.0.md` § "Bridge names (1.5.0)".
- **`SignalOptions`/`DeriveSignalOptions`/`SignalCallback` renamed to `CellOptions`/`DeriveCellOptions`/`CellCallback`, absorbing `ComputedOptions`/`SensorOptions`/`SensorCallback`** (`src/graph.ts`): Follow-on renames completing the shape-indexed naming of the `Cell` family's auxiliary types, now that `Signal` no longer means the narrow shape. `CellOptions` is used by `createState`/`createCell`/`createSlot`; `DeriveCellOptions` and `CellCallback` by `deriveCell`/`deriveComputed` — and now also cover what `ComputedOptions` (Memo/Task options) and `SensorOptions`/`SensorCallback` (external-push options) meant separately in 1.x, since those origins no longer have distinct public option types. Type-only renames, no runtime or bundle-size impact. **Migration:** rename `SignalOptions`/`DeriveSignalOptions`/`SignalCallback` to `CellOptions`/`DeriveCellOptions`/`CellCallback` at any import site; rename `ComputedOptions`/`SensorOptions` to `DeriveCellOptions` and `SensorCallback` to `CellCallback` at any site still using the pre-1.5.0 names.
...
- **`State<T>`/`isState(x)`, `Memo<T>`/`isMemo(x)`, `Task<T>`/`isTask(x)`, `Sensor<T>`/`isSensor(x)`, `Collection<T>`/`isCollection(x)`, `isComputed(x)`** (deprecated since 1.5.0, see that section's Deprecated table): Origin-indexed type names and guards have no referent once types are indexed by shape and mutability alone (ADR-0018) — `Cell`, `List`, and `Store` cover every case, and origin moved entirely to the construction verb (`create*`/`derive*`). No mechanical replacement exists for the guards. **Migration:** replace a type annotation with `Cell<T>`/`MutableCell<T>`, `List<T, S>`/`MutableList<T, S>`, or `Store<T>`/`MutableStore<T>`; replace a guard with `isSignal`/`isMutableSignal` (umbrella, matches any shape) or `isCell`/`isList`/`isStore` (shape-specific) — audit each call site by hand, since `isState(x) || isMemo(x)` does not mechanically become `isSignal(x)` once the value can be a composite. See `MIGRATION-2.0.md` § "Origin guards".
- **`createSignal`'s 1.x shape-sniffing coercion** (array → `List`, record → `Store`, function → `Memo`/`Task`, signal → itself): Removed with no replacement export — a coercion that hides the shape decision is exactly the v1-ism the shape-indexed taxonomy exists to delete. `createCell(value)`, the 2.0 name for the single-value factory, is single-value only and never coerces. **Migration:** call `createList`/`createStore`/`createState` directly for the shape you need, or use the three-line `coerceSignal` helper documented in `MIGRATION-2.0.md` if a coercing re-export is required.
- **`isObjectOfType`**: Deprecated in favor of `isSignalOfType`, which is now the sole signal-guard primitive (zero-allocation `Symbol.toStringTag` check). **Migration:** replace `isObjectOfType(value, tag)` with `isSignalOfType(value, tag)`.
- **`isEqual`**: Removed deprecated alias of `DEEP_EQUALITY`. **Migration:** use `DEEP_EQUALITY` directly.

### Fixed

- **`deriveStore(seed, { watched })` never started its watched lifecycle when consumed only through property reads, leaving the store frozen at its seed** (`src/nodes/store.ts`): The external-push facade delegated property reads to the backing mutable Store's `byKey`, which deliberately creates no structural edge (ADR-0015) — but nothing else activated the lifecycle either, so a store consumed solely via `store.prop.get()` / `store.byKey(k)` / `'prop' in store` never ran `watched` and never received a pushed value. The facade now owns activation through a dedicated lifecycle anchor node: any observation form starts `watched` exactly once and stops it when the last observer detaches, while per-property reads still create no structural edge, preserving granularity. Backported to 1.5.1.
- **`List` mutations now validate duplicate keys and nullish items atomically** (`src/nodes/list.ts`): `splice()` silently accepted the same content-based key twice in one inserted batch, corrupting `length` and iteration; it also validated items mid-mutation, so an invalid item could leave earlier insertions committed with no propagation. Insertions are now staged and validated up front — an invalid `splice` throws (`DuplicateKeyError` / `NullishSignalValueError`) with the list untouched. `createList()` and the external-push form of `deriveList()` now also reject duplicate keys in their initial value/seed instead of silently overwriting the first item's signal. Auto-increment keys (the default) are unaffected. Backported to 1.5.1.
- **`tools/codemod-v2.ts` rewrote `createComputed` to the sync-only `deriveComputed`, silently breaking any async call site** (`tools/codemod-v2.ts`): `createComputed` dispatches to `Task` for an async callback in 1.x, but `deriveComputed` — the shape-indexed narrow factory — accepts only a sync callback and throws otherwise. The codemod's `createComputed` → `deriveComputed` rewrite produced code that type-checked but threw at runtime for any consumer call site that passed an async callback, with no warning. Retargeted to `createComputed` → `deriveCell` (dispatches sync → `Memo`/`deriveComputed`, async → `Task`, matching `createComputed`'s actual 1.x behavior); a literal `options.value` is rewritten to `options.initial` to match `deriveCell`'s vocabulary, and a renamed async call site is flagged for the follow-up `.isPending()`/`.abort()` → free-function migration. `createMemo` → `deriveComputed` is unaffected — `createMemo` never accepted an async callback.
- **`deriveList`'s per-item overloads resolved an async item callback against the sync overload, inferring the item type as `Promise<T>` instead of the awaited `T`** (`src/nodes/list.ts`): TypeScript resolves overloads in declaration order; the sync `(sourceValue: U) => T` overload was declared before the async `(sourceValue: U, abortSignal) => Promise<T>` one, so an async item callback silently unified to the wrong overload. Reordered so the async overload is checked first. Forward-ported from 1.5.2.
- **`List.set()`/`deriveList` reused a `keyConfig`'d item's key across a content change at a shared index instead of retiring it, so a key could silently end up pointing at unrelated content** (`src/nodes/list.ts`): `diffPositional()` now mints a fresh key on a content mismatch when a `keyConfig` is given (matching `splice()`'s existing semantics); the no-`keyConfig` positional case — where array position is the identity contract — is unchanged. Forward-ported from 1.5.2.
- **The external-push form of `deriveList` silently dropped a `change`/`remove` entry it could not resolve to a key without a content-based `keyConfig`, instead of surfacing an error** (`src/nodes/list.ts`): `onChanges()` now resolves every entry in a `change`/`remove` batch before mutating anything, and throws the new `UnresolvableKeyError` if any entry can't be resolved — mirroring the existing add-batch staging. Forward-ported from 1.5.2.
