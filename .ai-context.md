...
│   │   └── collection.ts  # createCollection, createSourceCollection — derived and externally-driven collections
...
### Naming Conventions
- Factory functions: `create*` prefix (createState, createMemo, createEffect, createStore, createList, createCollection, createSourceCollection, createSensor)
...
- `unlink()` calls `source.stop()` when last sink disconnects (auto-cleanup for Sensor/SourceCollection/Store/List)
- AbortSignal integration for canceling async operations
- `createScope()` for hierarchical cleanup of nested effects

## Resource Management

**Sensor and SourceCollection** use a start callback that returns a Cleanup function:
```typescript
