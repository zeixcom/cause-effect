...
**Prefer deriving over writing imperatively.** When a signal's value should track another signal or an async source, use `deriveStore`, `deriveList`, or `deriveCell` (sync or async) rather than an effect that calls `.set()`/`.update()` on a target signal. `deriveStore(async (prev, abort) => …, { initial })` and `deriveList(source, itemCallback)` cover the common case of filling a Store or List from an async or another signal directly. See `deriving_vs_writing` in references/signal-types.md.
</essential_principles>
...
| File | Contents |
|---|---|
| ../shared/references/api-facts.md | Key API constraints, core functions, options, callback patterns |
| ../shared/references/non-obvious-behaviors.md | Counterintuitive behaviors with correct vs incorrect examples |
| ../shared/references/error-classes.md | Error classes, trigger conditions, and how to handle them |
