...
**Stale detection via `match()`**: Both the single-signal and tuple overloads of `match()` support an optional `stale` handler. After collecting values, if all unset signals would have routed to `nil` but there are none, and no errors exist, `match()` checks whether any signal is a Task with `isPending() === true`. If so, and a `stale` handler is provided, it is called as a thunk with no arguments. The cleanup it returns is the mechanism for resetting the stale display (e.g. hiding a spinner) — the effect's own cleanup cycle runs it before the next dispatch. If no `stale` handler is provided, falls back to `ok` (preserving current behavior). Routing precedence: `nil` > `err` > `stale` > `ok`. This is symmetrical with `nil`: `nil` fires if any signal is unset; `stale` fires if any signal is a pending Task with a retained value. `isPending()` itself remains a plain boolean check — no reactive tracking — because the downstream effect already re-runs at both transition points: when dependencies change (task goes pending) and when the promise resolves (task value changes).
...
|----------|--------|------------------------|-----------|
| Task stale detection in `match()` | Check `isPending()` (plain boolean) after a successful `get()`; route to `stale` handler if present | (a) Make `isPending()` reactive via an internal `StateNode<boolean>`; (b) dedicated `stale` state in `TaskNode` propagated through the graph | The downstream effect already re-runs at both transition points (dependency change → pending; resolution → new value), so reactive tracking of pending state is redundant. A plain check at match-time is sufficient and avoids adding a second propagation path to TaskNode. |
