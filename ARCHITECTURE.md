...
| Signal writes inside effects (v2.0, proposed, not accepted) | [ADR-0019](adr/0019-signal-writes-in-effects-throw-at-runtime.md) proposes: every public mutator (`MutableSignal.set/update`, the seven `List` mutators, the four `Store` mutators, `Slot.set`) throws `EffectWriteError` when called synchronously during an effect body — detected via `activeSink` being an `EffectNode` (`!('value' in node)`, the same discrimination `refresh()` uses). A module-level trusted-write window (a counter, like `batchDepth`) would exempt the library's own emit-driven writes, which legitimately run while an effect is active: sensor `emit` on first read, task `pendingNode` flips, and the `onChanges` handlers of external-push `deriveList`/`deriveStore`. `untrack()` would be the escape hatch; no new API | (a) ban only untracked writes (rejected in the ADR: an effect's dependencies are unknown until the run ends — write-before-read ordering — forcing deferred end-of-run reconciliation with worse stack traces, while keeping convergence handling for self-writes); (b) dev-mode-only guard (rejected: ESM-only, environment-agnostic library has no dev/prod split; precedent errors are always-on); (c) guard inside `setState` (rejected: List/Store structural writes bypass `setState` entirely, and internal callers would false-positive); (d) extend to memo/task bodies now, MobX-style (deferred: needs a false-positive audit of composite rebuild paths) | Not accepted. A prototype implementation and review (2026-08-16) found the guard as specified bans *every* synchronous effect-body write, not just writes that create the invisible dependency the ADR's Context motivates — it throws even when the effect never reads the written signal — and unlocks no simplification of the convergence machinery, which stays fully intact as the backstop either way. Parked pending a more targeted detection (e.g. scoped to signals the effect actually reads) or a stronger case for the broad rule on its own terms. See ADR-0019 Status. |
